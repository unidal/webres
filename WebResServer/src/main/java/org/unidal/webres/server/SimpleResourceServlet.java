...
/**
 * Stand-alone resource servlet for dev mode purpose.
 * <p>
 * 
 * No cache configured and only local file based resource repository will be
 * used.
 * <p>
 * 
 * This servlet needs to be configured in web.xml with following url-mapping:
 * <xmp> <servlet> <servlet-name>ResourceServlet</servlet-name>
 * <display-name>ResourceServlet</display-name>
 * <servlet-class>org.unidal.webres.server.SimpleResourceServlet</servlet-class>
 * <load-on-startup>1</load-on-startup> </servlet> <servlet-mapping>
 * <servlet-name>ResourceServlet</servlet-name> <url-pattern>/z/*</url-pattern>
 * </servlet-mapping> <servlet-mapping>
 * <servlet-name>ResourceServlet</servlet-name> <url-pattern>/f/*</url-pattern>
 * </servlet-mapping> </xmp>
 */
	...
	protected void handleLocalFile(HttpServletRequest req, HttpServletResponse res) throws IOException {
		...
		if (file.isFile()) {
			try {
				...
				String ext = pos > 0 ? path.substring(pos + 1) : "";
				...
				if (ext.equals("js")) {
					...
					renderPage(res, 200, "images/" + ext, content);
				} else if (ext.equals(".svg")) {
					renderPage(res, 200, "images/svg-xml", content);
				} else {
					renderPage(res, 200, "text/html", content);
				}
			} catch (Exception e) {
				handleException(req, res, e);
			}

...
public class TldGenerator {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < len - 1; i += 2) {
			...
			tldGenerator.generateTldFile(tldDir, args[i] + "_deprecated", args[i + 1], tldExceptions,
			      OLD_TAGLIB_URI_RESOURCE);
		}
	...
	public void generateTldFile(File tldDir, String fileName, String className, List<Exception> tldExceptions,
	      String newUri) {
		File tldFile = new File(tldDir, fileName + ".tld");

		try {
			BaseTagLibDefinition tldef = getInstanceOfDefinition((Class<? extends BaseTagLibDefinition>) Thread
	...
	protected BaseTagLibDefinition getInstanceOfDefinition(Class<? extends BaseTagLibDefinition> clz) throws Exception {
		...
		try {
			...
			final Object[] params = new Object[] { tagLibUri, tagMeta.shortName(), tagMeta.description(),
			      tagMeta.version(), tagMeta.jspVersion() };
	...
	protected final CDATASection newCDATASection(Node node, String name) {
		CDATASection cds = node.getOwnerDocument() == null ? ((Document) node).createCDATASection(name) : node
		      .getOwnerDocument().createCDATASection(name);
	...
	protected final Element newElement(Node node, String name, Object value) {
		Element e = node.getOwnerDocument() == null ? ((Document) node).createElement(name) : node.getOwnerDocument()
		      .createElement(name);
	...
	protected void tldefToDom(BaseTagLibDefinition tldef, Document document) throws DOMException,
		...
		List<TagInfo> tags = new ArrayList<TagInfo>(tldef.getTags().values());
		List<TagFileInfo> tagFiles = new ArrayList<TagFileInfo>(tldef.getTagFiles().values());
		List<FunctionInfo> functions = new ArrayList<FunctionInfo>(tldef.getFunctions().values());
